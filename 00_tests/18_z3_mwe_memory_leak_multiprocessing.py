...
failed_runs = {}
def overloaded_submitter_work(problems, verbose=0, print_prefix=''):
    print(print_prefix + 'problems', problems)
    failed_runs = []
    answers = []
    tmp_ans = None
    for p in problems:
        try:
            print(print_prefix + 'start working on problem {}', p)
            sampler = DWaveSampler(token = token, architecture='pegasus', region='eu-central-1')
            if p == 42:
                raise NotImplementedError('for the purpose of testing, problem {} raised an exception. Let us see what happens'.format(p))
            #composite = FixedEmbeddingComposite(child_sampler=sampler, embedding=embedding)
            tmp_ans = '42 is probably not the answer to problem {}'.format(p)
            print(print_prefix + 'succesfully retrieved answer for problem {}', p)
        except Exception as e:
            print(print_prefix + 'Error in process {}:'.format(multiprocessing.current_process().name), e)
            failed_runs.append(problems)
            tmp_ans = e
        finally:
            answers.append(tmp_ans)
    print('number of failed runs in process {}: {}'.format(multiprocessing.current_process().name, len(failed_runs)), ' Failed runs:', failed_runs)
    return answers
    
def overloaded_writer_work(answer, verbose=0, print_prefix=''):
