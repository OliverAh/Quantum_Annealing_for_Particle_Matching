...
def _main_update_study_in_info_file(folder_path_main, info_file_name, old_info_file_name):
    ...
    names = ['anneal_offsets_1_qubits', 'anneal_offsets_2_qubits', 'anneal_offsets_3_qubits']
    salib_problem = {
    'num_vars': 3,
    'names': names,
    ...
    anneal_offsets = np.zeros((study.shape[0], len(anneal_offsets)))
    for i in range(anneal_offsets.shape[1]):
        anneal_offsets[:,i] = i
    study_explicit = anneal_offsets
    names_explicit = ['anneal_offsets']

    #study = study_explicit
    ...
    for i in range(study.shape[0]):
        params_sampling = {'label' : 'superdupernice label',
                  'num_reads': 1000, 
                  'answer_mode': 'raw'}
        #params_sampling.update({names[j]: study[i,:] for j in range(len(names))})
...
def _main_determine_anneal_offset():
    folder_path_main = os.path.join(os.getcwd(), '01_out', 'sub_5_2')
    info_file_name = 'study_params_sub5_2.h5'
...
if __name__ == '__main__':
    ...
    #    folder_path_main=r'/Users/adam-1aeqn8vhvpjnv4u/mast_sub5/Quantum_Annealing_for_Particle_Matching/00_tests/01_out/sub_5_2',\
    #    info_file_name='study_params_sub5_2.h5',\
